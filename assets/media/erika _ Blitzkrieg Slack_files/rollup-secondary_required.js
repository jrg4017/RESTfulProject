...
},sendMsg:function(k,m){var h;
var i=TS.channels.getChannelById(k);
if(!i){return false
}if(i.is_archived){return false
}if(!i.is_member){return false
}var f=TS.channels.getGeneralChannel();
var l=function(o,n){if(n){TS.client.ui.addOrFlashEphemeralBotMsg({text:o,ephemeral_type:n})
}else{TS.generic_dialog.alert(o)
}TS.client.ui.$msg_input.val(m);
TS.view.focusMessageInput()
};
if(i.is_general&&!TS.members.canUserPostInGeneral()){l("A team owner has restricted posting to the #*"+i.name+"* channel.","general_posting_restricted");
return false
}var g=TS.format.cleanMsg(m);
var j=TS.boot_data.feature_at_here&&TS.model.here_regex.test(g);
var e=TS.model.channel_regex.test(g)||TS.model.group_regex.test(g)||j;
if(TS.model.everyone_regex.test(g)||(i.is_general&&e)){if(!TS.members.canUserAtEveryone()){h="<p>A team owner has restricted the use of <b>@everyone</b> messages.</p>";
if(TS.model.user.is_restricted){h="<p>Your account is restricted, and you cannot send <b>@everyone</b> messages.</p>"
}if(!i.is_general&&TS.members.canUserAtChannelOrAtGroup()){h+='<p class="no_bottom_margin">If you just want to address everyone in this channel, use <b>@channel</b> instead.</p>'
}l(h);
return false
}if(!i.is_general&&!TS.model.here_regex.test(g)){if(!f||!f.is_member){h="<p>You cannot send <b>@everyone</b> messages.</p>";
...
},upsertFile:function(x){if(x.mode=="space"){try{if(x.preview){var w=$(x.preview);
...
(function(){TS.registerModule("shared",{msg_sent_sig:new signals.Signal(),onStart:function(){},calcUnreadCnts:function(a,g,q){a.unreads.length=0;
...
}p=TS.utility.msgs.msgCanCountAsUnread(b);
o=o||p;
if(!p){continue
}a.unreads.push(b.ts);
...
a.unread_highlights.length=0;
a.oldest_unread_ts=null;
if(q){g.markMostRecentReadMsg(a,TS.model.marked_reasons.none_qualify)
...
if(H.file.mode=="snippet"){x+=TS.templates.message_file_snippet_create(n)
...
n.uploader=M
}if(H.file.mode=="snippet"){x+=TS.templates.message_file_snippet_share(n)
...
var q;
if(d.internal_file_id){if(p&&p.mimetype.indexOf("image/")===0){if(p.external_type=="dropbox"||p.external_type=="gdrive"||p.external_type=="box"||p.external_type=="onedrive"){var i=p.thumb_720?p.thumb_720:p.thumb_360;
...
if(p&&TS.boot_data.feature_fix_files){if(TS.client){e+='<i class="ts_icon ts_icon_arrows_alt preview_main_action"></i>'
}}e+="</a>";
...
}else{if(/^@here[\.|\-|_]*$/.test(f)&&TS.boot_data.feature_at_here){g="<!here>";
...
},formatWithOptions:function(r,m,q){var k=(q&&!!q.do_inline_imgs)||undefined;
var w=(q&&!!q.for_growl)||undefined;
var p=(q&&!!q.for_edit)||undefined;
var g=(q&&!!q.no_highlights)||undefined;
var y=(q&&!!q.no_specials)||undefined;
var v=(q&&!!q.enable_slack_action_links)||undefined;
g=(m&&("no_highlights" in m))?!!m.no_highlights:!!g;
if(y===true||y===false){y=y
}else{if(m&&("mrkdwn" in m)){y=(m.mrkdwn===false)
}else{y=false
}}if(p){y=true
}var s=(m&&m.no_emoji);
var u=!(k&&(!m||m.subtype!="bot_message"));
var e=(!u&&TS.client&&TS.model.team&&TS.model.team.domain=="tinyspeck");
if(!r){return""
}var x=$.trim(r);
if(!x){return""
}var n;
...
}n=c(r,m,f,y,g,s,u,k,v,e)
}catch(l){TS.error("error testing TSF:"+l);
TS.info("txt was:"+r)
}}var j=[];
var i=[];
var t=function(U,B,F,T){var X;
var R;
var M;
var Z;
if(B.substr(0,1)=="#"){var L=B.substr(1);
M=L.split("|");
R=M[0];
X=TS.channels.getChannelById(R);
if(!X){X=TS.channels.getChannelByName(R)
}if(X){if(w||p){return TS.format.tokenizeStr(j,"#"+X.name)
}Z=TS.utility.shouldLinksHaveTargets()?'target="/archives/'+X.name+'"':"";
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(j,TS.format.generic_link_open+"#"+X.name+TS.format.link_close)
}return TS.format.tokenizeStr(j,'<a href="/archives/'+X.name+'" '+Z+' data-channel-name="'+X.name+'" data-channel-id="'+X.id+'" class="internal_channel_link">#'+X.name+TS.format.link_close)
}else{if(M.length>1&&M[1]){return TS.format.tokenizeStr(j,"#"+M[1])
}else{if(TS.model.user.is_restricted){return TS.format.tokenizeStr(j,"#unknown-channel")
}else{return TS.format.tokenizeStr(j,"#deleted-channel")
}}}}if(B.substr(0,1)=="@"){U=TS.utility.msgs.getMemberFromMemberMarkup(B);
if(U){if(w||p){return TS.format.tokenizeStr(j,"@"+U.name)
}Z=TS.utility.shouldLinksHaveTargets()?'target="/team/'+U.name+'" ':"";
var aa=(g)?"@"+U.name:TS.format.doHighlighting("@"+U.name);
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(j,TS.format.generic_link_open+aa+TS.format.link_close)
}return TS.format.tokenizeStr(j,'<a href="/team/'+U.name+'" '+Z+'data-member-name="'+U.name+'" class="internal_member_link">'+aa+TS.format.link_close)
}else{return TS.format.tokenizeStr(j,B)
}}if(B.substr(0,1)=="!"){var V=B.substr(1);
var Q=(V||"").split("|");
V=Q[0];
var O=(Q[1])?Q[1]:V;
if(TS.utility.inArray(b,V)){if(w||p){return TS.format.tokenizeStr(j,"@"+V)
}return TS.format.tokenizeStr(j,'<b class="mention">@'+V+"</b>")
}else{if(V.indexOf("^")!==-1){var I=V.split("^");
V=I[0];
if(V==="date"&&I.length>=3&&TS.boot_data.feature_client_date_formatting){var S=I[1];
var ab=I[2];
var N=(I.length>3)?I[3]:"";
var D=(N===""||w||p)?"":"<a "+TS.utility.makeRefererSafeLink(N)+' target="_blank">';
var z=(N===""||w||p)?"":TS.format.link_close;
return TS.format.tokenizeStr(j,D+TS.utility.date.formatDate(ab,S,O)+z)
}else{return TS.format.tokenizeStr(j,O)
}}}if(w||p){return TS.format.tokenizeStr(j,"<"+O+">")
}else{return TS.format.tokenizeStr(j,"&lt;"+O+"&gt;")
}}var Y=B.split("|");
var C=Y.shift();
C=C.replace(/\"/g,"&quot;");
var H=Y.join("|")||C;
H=$.trim(H);
if(C.indexOf("<")===0){return TS.format.tokenizeStr(j,"&lt;"+B.replace(/</g,"&lt;").replace(/>/g,"&gt;")+"&gt;")
}H=H.replace(/</g,"&lt;");
H=H.replace(/>/g,"&gt;");
if(w){if(H==C){var J=(C.indexOf("//")>-1)?C.split("//")[1]:C;
J=jQuery.trim(J).substring(0,30).trim(this)+"...";
return""+J+""
}return"<"+H+">"
}else{if(p){return H
}}if(!y&&H!=C){H=TS.format.doSpecials(H,m&&m._special_debug)
}if(!s&&H!=C){H=TS.utility.emojiReplace(H,i)
}if(!g){H=TS.format.doHighlighting(H)
}var G;
if(C.indexOf(TS.utility.msgs.api_url_prefix+"chat.help")===0){if(v){G=TS.utility.htmlEntities(TS.utility.jsString(C));
return TS.format.tokenizeStr(j,'<a onclick="TS.utility.msgs.doApiUrl('+G+')" class="api_url">'+H+TS.format.link_close)
}else{return TS.format.tokenizeStr(j,'<a class="api_url muted">(Disabled) '+H+TS.format.link_close)
}}else{if(C.indexOf(TS.utility.msgs.new_api_url_prefix)===0){if(v){G=TS.utility.htmlEntities(TS.utility.jsString(C));
return TS.format.tokenizeStr(j,'<a onclick="TS.utility.msgs.doNewApiUrl('+G+')" class="api_url">'+H+TS.format.link_close)
}else{return TS.format.tokenizeStr(j,'<a class="api_url muted">(Disabled) '+H+TS.format.link_close)
}}else{if(C.indexOf("javascript:")==0){return TS.format.tokenizeStr(j,'<a onclick="'+C.replace("javascript:","")+'">'+H+TS.format.link_close)
}else{if(TS.client&&TS.client.core_url&&C.indexOf(TS.client.core_url)===0){if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(j,TS.format.generic_link_open+H+TS.format.link_close)
}return TS.format.tokenizeStr(j,'<a target="_self" href="'+C+'">'+H+TS.format.link_close)
}else{var W="";
var K;
if(m&&m.ts&&k){K=TS.inline_attachments.getAttachmentByFromUrl(m.attachments,C);
if(K){if(TS.boot_data.feature_attachments_inline||TS.templates.builders.shouldDoSimpleAttachment(K,m)){W=TS.templates.builders.buildAttachmentHTML({attachment:K,url:C,msg:m,show_initial_caret:true}).replace(/\n/g,"").replace(/\t/g,"").replace(/  /g," ")
}}}if(W){W=W.replace(/\n/g,"").replace(/\t/g,"").replace(/ +/g," ")
}if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(j,TS.format.generic_link_open+H+TS.format.link_close+W)
}var A=TS.utility.getFileIDFromURL(C);
var E=(A)?' class="file_preview_link"':"";
var P=(A)?' data-file-id="'+A+'"':"";
return TS.format.tokenizeStr(j,"<a "+TS.utility.makeRefererSafeLink(C)+' target="_blank"'+E+P+">"+H+TS.format.link_close+W)
}}}}};
x=x.replace(/<(.*?)>/g,t);
x=x.replace(/\</g,"&lt;");
x=x.replace(/\>/g,"&gt;");
if(p){x=TS.utility.unHtmlEntities(x)
}else{if(w){if(TS.utility.platformSupportsHtmlNotifications()){if(TS.utility.platformSupportsImgEmojiInHtmlNotifications()){x=TS.utility.emojiReplace(x,null,true)
}}else{x=TS.utility.unHtmlEntities(x);
if(emoji.replace_mode=="unified"){x=emoji.replace_colons_with_unified(x)
}}}else{TS.format.special_token_map=[];
if(!y){x=TS.format.doSpecials(x,m&&m._special_debug)
}if(!u){if(e){x=x.replace(TS.format.theme_rx,function(A,z,B){return z+"<nobr>"+B+" "+TS.format.tokenizeStr(TS.format.special_token_map,a(A)+"</nobr>")
})
}x=x.replace(TS.format.hex_rx,TS.format.hexReplace)
}if(!s){x=TS.utility.emojiReplace(x,i)
}x=TS.format.deTokenizeStr(TS.format.special_token_map,x);
TS.format.special_token_map=null;
if(!g){x=TS.format.doHighlighting(x)
}x=x.replace(/<\/div>\n/g,"</div>");
x=x.replace(/<\/pre>\n/g,"</pre>");
x=x.replace(/codecopyonly> /g,"codecopyonly>&nbsp;");
x=x.replace(/ <span class="codecopyonly/g,'&nbsp;<span class="codecopyonly');
x=x.replace(/&nbsp;&nbsp;/g," &nbsp;");
...
x=x.replace(/  /g," &nbsp;");
x=x.replace(/^ /g,"&nbsp;")
}}x=TS.format.deTokenizeStr(j,x);
x=TS.format.deTokenizeStr(i,x);
i=null;
if(window.TSF&&n){if(n!=x){TS.error("======================================================================== "+f+" "+(n==x));
TS.info("txt     :"+r);
TS.info("new_txt :"+x);
TS.info("tsf_str :"+n);
if(TS.model.user.id=="U024BE7LM"||TS.model.user.id=="U032VEM0B"){window._temp_bad_message_hash=window._temp_bad_message_hash||{};
var h="format bot";
var o=(m)?m.ts:r.substr(0,30);
if(!window._temp_bad_message_hash[o]&&(!m||m.username!=h)){window._temp_bad_message_hash[o]=true;
TS.ims.addMsg(TS.ims.getImByMemberId("USLACKBOT").id,{type:"message",subtype:"bot_message",is_ephemeral:true,username:h,icons:{emoji:":symbols:"},ts:TS.utility.date.makeTsStamp(),text:"check console for bad message report! ts: "+(m?m.ts:"unknown")})
}}}else{if(window.show_all_msg_tests){TS.warn("================================"+f+" "+(n==x));
TS.info("new_txt:"+x);
TS.info("tsf_str:"+n)
}return n
}}return x
...
return'<a data-theme="'+TS.utility.htmlEntities(f)+'" class="btn btn_small tiny_right_margin theme_installer_btn" style="padding: 0px 7px;"><i class="ts_icon ts_icon_cog"></i> Switch'+e+"</a>"
}
})();
...
TS.members.members_for_user_changed_sig.add(TS.menu.membersForUserChanged,TS.menu);
...
},membersForUserChanged:function(){TS.menu.members_html_cache=null;
if(!TS.model.menu_is_showing){return
...
TS.members.members_for_user_changed_sig.remove(TS.menu.membersForUserChanged,TS.menu);
